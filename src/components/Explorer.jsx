import { Box, Breadcrumb, Icon, Link, Spinner, Table, VStack } from "@chakra-ui/react";
import PropTypes from "prop-types";
import React, { Fragment, useEffect } from "react";
import { GrDocument, GrFolder, GrHome } from "react-icons/gr";
...
function Navigation({ prefix }) {
    ...
    return (
        <Breadcrumb.Root size="lg" borderWidth="1px" shadow="md" p={2} background="gray.100">
            <Breadcrumb.List>
                ...
                {folders.map((item) => (
                    <Fragment key={item.url}>
                        ...
                        <Breadcrumb.Item>
                            {item.isCurrent ? (
                                <Breadcrumb.CurrentLink fontWeight="bold">{item.name}</Breadcrumb.CurrentLink>
                            ) : (
                                    ...
                                    <>
                                        ...
                                        {data?.objects.map((item) => (
                                            <Table.Row key={item.path}>
                                                ...
                                                <Table.Cell>{item.lastModified.toLocaleString()}</Table.Cell>
                                                <Table.Cell textAlign="right">{formatFileSize(item.size)}</Table.Cell>
                                            </Table.Row>
                                        ))}
                                    </>
                                );
